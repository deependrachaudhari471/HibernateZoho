...
import in.com.prestige.dto.EventDTO;
...
public class TestZoho {
	...
	public static void addRecords() throws Exception {
		...
		try {
			...
			project.setActivity(activity); // ActivityDTO activity type
            //Activity SET for Feed
			Set actSet = new HashSet();
			actSet.add(activity);
			feed1.setActivity(actSet);
			//feed2.setActivity(actSet);
			...
			shareBug1.setShareBugName(bug1.getDescription());
			// ShareBugDTO for UserDTO manyToMany-1
			ShareBugDTO shareBug2 = new ShareBugDTO();
			shareBug2.setShareBugId(4445);
			shareBug2.setDate(new Date());
			shareBug2.setShareBugName(bug2.getDescription());
  
			//EventDTO
			EventDTO event = new EventDTO();
			event.setEventId(9999);
			event.setEvent_title("Rahul's BirthDay Party");
			event.setDurations("3 Hours");
			event.setLocation("Saket Indore");
			event.setScheduleDate(new Date());
			
			//EventDTO
			EventDTO event1 = new EventDTO();
			event1.setEventId(9998);
			event1.setEvent_title("Water park Event");
			event1.setDurations("6 Hours");
			event1.setLocation("Cresent Park Indore");
			event1.setScheduleDate(new Date());
			...
			milestone1.setBugs(bugSet);
			milestone2.setBugs(bugSet);
			...
			user2.setEvents(eventSet);
			manager1.setEvents(eventSet);
			manager2.setEvents(eventSet);
